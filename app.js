...
async function shareVerse(bookName, bookSlug, chapter, verse) {
  const url = `${location.origin}/${bookSlug}/${chapter}#v${verse}`;
  const text = `${bookName} ${chapter}:${verse}`;

  try {
    if (navigator.share) {
      await navigator.share({ title: "La Bible", text, url });
      return;
    }
  } catch (_) {
...
async function init() {
  ...
  $("content").addEventListener("click", (e) => {
    ...
    shareVerse(bookObj.name, bookObj.slug, chapter, verse);
  });
...
function render(bookNum, chapNum, opts = { updateURL: true }) {
  ...
  // scroll to #vXX if present
  if (location.hash && location.hash.startsWith("#v")) {
    const el = document.querySelector(location.hash);
    if (el) el.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  // share icon hint: close search results when reading
  $("results").classList.add("hidden");
}
